...
EOF
    ...
    *fish*)
      ...
      if [ -f "$old_rcfile" ] && grep --quiet -F "### Do not edit. This was autogenerated by 'asdf cmd direnv setup' ###" "$old_rcfile"; then
        rm "$old_rcfile"
      fi
...
function print_usage() {
  echo "Usage: asdf cmd direnv setup [--no-touch-rc-file] [--shell SHELL] [--version VERSION]"
  echo ""
...
function set_command() {
  ...
  while [ $# -gt 0 ]; do
    ...
    maybe_run_cmd asdf install "$plugin" "$version"
    run_cmd asdf set "$plugin" "$version"
  done
