...
_load_asdf_lib() {
  ...
  if [ -z "$(declare -f -F "$expected_function")" ]; then
    ...
    if [ ! -f "$lib_file" ]; then
      log_error "Could not find asdf $path file in $ASDF_DIR checking direnv copy"
      lib_file="$(dirname "${BASH_SOURCE[0]}")/../lib-asdf/$path"
      if [ ! -f "$lib_file" ]; then
        log_error "Could not find asdf $path file in cached $lib_file"
        return 1
      fi
    fi
...
_asdf_envrc() {
  local tools_file="$1"
  cat <<-EOF
### Do not edit. This was autogenerated by 'asdf cmd direnv envrc' from $tools_file ###
EOF
...
_load_plugin_version_and_file() {
  ...
  while IFS=$'\n' read -r version_and_path; do
    ...
    _plugin_env_bash "$plugin_name" "$version" "$plugin_name $version not installed. Run 'asdf cmd direnv install' to install."
    if [ -f "$path" ]; then
      printf 'watch_file %q\n' "$path"
    fi
